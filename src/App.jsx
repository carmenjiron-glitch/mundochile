...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",minHeight:"100vh",background:"linear-gradient(135deg, #1a2a4a 0%, #1e3a6e 50%, #2563a8 100%)",color:"#FFFFFF",WebkitFontSmoothing:"antialiased",MozOsxFontSmoothing:"grayscale",textRendering:"optimizeLegibility",overflowX:"clip"}}>
      ...
      {modalNuevoCli&&<ModalNuevoCliente onGuardar={async(d)=>{const{notas,...dSinNotas}=d;const{data}=await sb.from("clientes").insert(dSinNotas).select().single();if(data&&notas!==undefined)await sb.from("clientes").update({notas}).eq("id",data.id);if(data)setClientes(prev=>[...prev,data]);const cb=modalNuevoCli?.cb;setModalNuevoCli(false);if(data){cb?.(data.id);addToast("Cliente creado","success");}cargarDatos();}} onCerrar={()=>setModalNuevoCli(false)}/>}
