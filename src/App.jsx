...
const B_TIPO={"Simultánea":{bg:"#EEF2FF",c:"#3B5BDB",ct:"#294099"},"Consecutiva":{bg:"#FCE4EC",c:"#C2185B",ct:"#881140"},"Whispering":{bg:"#F3E5F5",c:"#7B1FA2",ct:"#561671"}};
const B_MOD={"presencial":{bg:"#E8F5E9",c:"#2E7D32",ct:"#205823"},"remoto":{bg:"#E0F7FA",c:"#00838F",ct:"#005C64"},"hibrido":{bg:"#FBE9E7",c:"#BF360C",ct:"#862608"}};
    ...
    <div style={{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)",padding:"16px",boxSizing:"border-box"}}>
      <div style={{background:"#FFFFFF",borderRadius:"20px",width:"100%",maxWidth:"760px",maxHeight:"90vh",display:"flex",flexDirection:"column",boxShadow:"0 24px 80px rgba(0,0,0,0.25)",position:"relative",zIndex:1001}}>
        ...
        <div style={{overflowY:"auto",flex:1,padding:"24px 28px"}}>
          ...
          {esMultidia&&dias.length>0&&<div style={{display:"flex",alignItems:"center",gap:"8px",fontSize:"13px",fontWeight:"600",textTransform:"uppercase",letterSpacing:"0.06em",color:"#111827",margin:"16px 0 8px 0"}}>
        ...
        <div id="ficha-exportable" ref={fichaRef} style={{flex:1,overflow:"visible",maxHeight:"none",height:"auto",background:"#F0F4FA",padding:"12px"}}>
          <div style={{width:"800px",maxWidth:"100%",margin:"0 auto",background:"#FFFFFF",border:"2px solid #1A6FD4",borderRadius:"12px",overflow:"hidden",fontFamily:"'Inter','Segoe UI',system-ui,sans-serif"}}>
            ...
            {(()=>{
              ...
              const asigsSingle=esMultidia?[]:(evento.asignaciones||[]);
              const interpsEl=campos.interpretes?renderI(asigsSingle):null;
              const hasInterps=interpsEl||(campos.interpretes&&esMultidia&&dias.length>0);
              ...
              return(<div style={{display:"flex",alignItems:"stretch"}}>
                ...
                <div style={{flex:1,minWidth:0,borderRight:"2px solid #D1D5DB"}}>
                  ...
                  {campos.comentarios&&evento.comentarios&&<><div style={sH}>Comentarios</div><div style={{...sB,borderBottom:"none"}}><div style={{fontSize:"13px",color:"#0a0f1d",lineHeight:1.5}}>{evento.comentarios}</div></div></>}
                </div>
                {/* Columna derecha — tipo/modalidad + intérpretes */}
                <div style={{width:"40%",flexShrink:0,minWidth:0}}>
                  {(campos.tipo||campos.modalidad)&&<><div style={{...sH,textAlign:"center"}}>Tipo / Modalidad</div><div style={{...sB,borderBottom:hasInterps?"1px solid #E5E7EB":"none"}}><div style={{display:"flex",gap:"8px",flexWrap:"wrap",alignItems:"center",justifyContent:"center"}}>{campos.tipo&&evento.tipo&&<span style={{display:"inline-flex",alignItems:"center",gap:"4px",padding:"5px 12px",borderRadius:"20px",fontSize:"14px",fontWeight:"500",color:(B_TIPO[evento.tipo]||{ct:"#294099"}).ct,WebkitFontSmoothing:"antialiased",background:(B_TIPO[evento.tipo]||{bg:"#EEF2FF"}).bg,border:`2px solid ${(B_TIPO[evento.tipo]||{c:"#3B5BDB"}).c}`,whiteSpace:"nowrap"}}>{evento.tipo==="Simultánea"?<IconoSimultanea/>:evento.tipo==="Consecutiva"?"🎤":"🤫"} {evento.tipo}</span>}{campos.modalidad&&evento.modalidad&&<span style={{display:"inline-flex",alignItems:"center",padding:"5px 12px",borderRadius:"20px",fontSize:"14px",fontWeight:"500",color:(B_MOD[evento.modalidad]||{ct:"#4B4B4B"}).ct,WebkitFontSmoothing:"antialiased",background:(B_MOD[evento.modalidad]||{bg:"#F7F7F5"}).bg,border:`2px solid ${(B_MOD[evento.modalidad]||{c:"#6B6B6B"}).c}`,whiteSpace:"nowrap"}}>{evento.modalidad==="presencial"?"📍":evento.modalidad==="hibrido"?"🔀":"💻"} {LBL_MODAL[evento.modalidad]}</span>}</div></div></>}
                  {interpsEl&&<><div style={{...sH,textAlign:"center"}}>Intérpretes</div><div style={{...sB,borderBottom:hasInterpsDia?"1px solid #E5E7EB":"none"}}>{interpsEl}</div></>}
