        ...
        <div style={{background:"#FFFFFF",padding:"20px 24px",borderRadius:"20px 20px 0 0",flexShrink:0,borderBottom:`10px solid ${colorCliente(evento.cliente_id)}`,position:"sticky",top:0,zIndex:10,boxShadow:"0 2px 8px rgba(0,0,0,0.08)"}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:"16px"}}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:"21px",fontWeight:"600",color:"#0F172A",lineHeight:1.2}}>{cliente?.nombre_empresa||"—"}</div>
              {cliente?.nombre_contacto&&<div style={{fontSize:"12px",fontWeight:"500",color:"#6B7280",fontStyle:"italic",marginTop:"4px"}}>Contacto: {cliente.nombre_contacto}</div>}
  ...
  const renderGrupos=(asignaciones)=>{
    ...
    return Object.entries(grupos).map(([key,grupo])=>{
      ...
      return(<div key={key} style={{marginBottom:"12px"}}>
        ...
        {grupo.items.map(({asig},i)=>(asig.nro_ot||asig.nro_boleta)?<div key={i} style={{fontSize:"13px",color:"#6B7280",marginTop:"4px",display:"flex",gap:"10px",flexWrap:"wrap"}}>{asig.nro_ot&&<span>OT: {asig.nro_ot}</span>}{asig.nro_boleta&&<span>Boleta: {asig.nro_boleta}</span>}</div>:null)}
      </div>);
    });
  };
        ...
        <div id="ficha-mc" ref={fichaRef} style={{flex:1,overflowY:"auto",padding:"16px 20px 20px",background:"#F8FAFC"}}>
          ...
          <div style={{display:"flex",alignItems:"center",height:"64px",padding:"0 16px",borderRadius:"8px",background:"#1E3A6E",color:"#fff",marginBottom:"12px",flexShrink:0}}>
            <div style={{flexShrink:0,marginRight:"12px",display:"flex",alignItems:"center"}}>
              <div style={{width:"48px",height:"48px",borderRadius:"50%",background:"#FFFFFF",overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 2px 8px rgba(0,0,0,0.15)",margin:"8px"}}>
                <img src={LOGO_SRC} alt="MundoChile" style={{width:"42px",height:"42px",objectFit:"contain"}}/>
              </div>
            </div>
            <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 12px"}}>
            ...
            {campos.facturacion&&<Sec label="Estado de facturación" accent="#1565C0" fullWidth>{(()=>{const be=B_EST(evento.estado);return<span style={{display:"inline-flex",alignItems:"center",padding:"4px 10px",borderRadius:"20px",fontSize:"12px",fontWeight:"500",lineHeight:"1.4",color:be.c,background:be.bg,border:`2px solid ${be.b||be.c}`,whiteSpace:"nowrap"}}>{evento.estado==="Facturado"?"✓ Facturado":"🟠 Facturación Pendiente"}</span>;})()}</Sec>}
      ...
      <div style={{padding:"10px 12px 80px"}}>
        ...
        {diasLF.map((d,i)=>{
          ...
          return <div key={i} style={{marginBottom:"10px",background:colBg,borderRadius:"12px",overflow:"hidden"}}>
            ...
            {evs.length>0&&<div style={{padding:"0 10px 10px"}}>{evs.map(ev=><EventCard key={ev.id} ev={ev} diaDe={iso} clientes={clientes} pares={pares} interpretes={interpretes} proveedores={proveedores} onClick={()=>abrirEvento(ev)} onNavegar={d=>{setDiaActual(d);setVista("dia");}} solidPill/>)}</div>}
          </div>;
        })}
      </div>
    );
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:"8px",padding:"8px"}}>
          {diasLF.map((d,i)=>{
            ...
            return <div key={i} style={{background:colBg,backdropFilter:"blur(4px)",WebkitBackdropFilter:"blur(4px)",borderRadius:"12px",padding:"10px",minHeight:"calc(100vh - 260px)"}}>
              ...
              {evs.map(ev=><EventCard key={ev.id} ev={ev} diaDe={iso} clientes={clientes} pares={pares} interpretes={interpretes} proveedores={proveedores} onClick={()=>abrirEvento(ev)} onNavegar={d=>{setDiaActual(d);setVista("dia");}} solidPill/>)}
