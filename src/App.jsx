    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.75)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(6px)",padding:"16px",overflowY:"auto"}}>
      <div style={{background:"#F8FAFF",borderRadius:"20px",width:"100%",maxWidth:"820px",maxHeight:"92vh",boxShadow:"0 8px 32px rgba(45,140,255,0.15)",display:"flex",flexDirection:"column"}}>
        ...
        <div id="ficha-exportable" ref={fichaRef} style={{flex:1,overflowY:"auto",background:"#F8FAFF",padding:"10px 12px 12px"}}>
          ...
          {(()=>{
            const AC="#2E7BC4";
            const dk10=(h)=>{const n=h.replace("#","");const r=Math.round(parseInt(n.slice(0,2),16)*0.9);const g=Math.round(parseInt(n.slice(2,4),16)*0.9);const b=Math.round(parseInt(n.slice(4,6),16)*0.9);return`#${r.toString(16).padStart(2,"0")}${g.toString(16).padStart(2,"0")}${b.toString(16).padStart(2,"0")}`;};
            ...
            return(
              <div style={{border:"3px solid #1A6FD4",borderRadius:"0 0 8px 8px",overflow:"hidden",display:"grid",gridTemplateColumns:"1fr 1fr",alignItems:"start"}}>
                ...
                {showFH?<div style={lB}><div style={{display:"flex",gap:"12px",flexWrap:"wrap",alignItems:"center"}}>{campos.fecha&&<div style={{fontSize:"14px",fontWeight:"500",color:"#0a0f1d"}}>{esMultidia?`${formatCorto(evento.fecha_inicio)} → ${formatCorto(evento.fecha_termino)}`:formatLargo(evento.fecha_inicio)}</div>}{campos.horario&&<div style={{fontSize:"14px",fontWeight:"500",color:"#0a0f1d"}}>🕐 {evento.hora_inicio?.slice(0,5)} – {evento.hora_termino?.slice(0,5)} hrs</div>}</div></div>:<div style={eS}/>}
                {showCo?<div style={rB}><div style={{fontSize:"13px",color:"#0a0f1d",lineHeight:1.5}}>{evento.comentarios}</div></div>:<div style={eS}/>}
