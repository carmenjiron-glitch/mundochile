...
function VistaGrilla({eventos,clientes,interpretes,pares,proveedores=[],contactos=[],onAbrir,onVerMultidia,vista}) {
  ...
  useEffect(()=>{if(vista!=="grilla")return;setTimeout(()=>{const hoy=new Date();const todayStr=`${hoy.getFullYear()}-${String(hoy.getMonth()+1).padStart(2,"0")}-${String(hoy.getDate()).padStart(2,"0")}`;let el=document.getElementById(`grilla-evento-${todayStr}`);if(!el){const filas=Array.from(document.querySelectorAll("[id^='grilla-evento-']"));el=filas.find(f=>{const fecha=f.id.replace("grilla-evento-","");return fecha>=todayStr;})||filas[0];}if(el){const offset=160;const top=el.getBoundingClientRect().top+window.scrollY-offset;window.scrollTo({top,behavior:"instant"});}},300);},[vista]);
      ...
      {fechas.map(fecha=>{
        ...
        return (
          <div key={fecha} id={idDia} data-fecha={fecha} style={{marginBottom:"32px"}}>
            ...
            {evs.length===0
              ?<div style={{padding:"16px 20px",color:"#646870",fontSize:"14px",fontStyle:"normal",display:"flex",alignItems:"center",gap:"8px",background:"#FAFAFA",borderRadius:"8px",border:"1px dashed #E5E7EB",marginBottom:"8px"}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#22C55E" strokeWidth="2.4"><circle cx="12" cy="12" r="10"/></svg>Sin eventos este día</div>
