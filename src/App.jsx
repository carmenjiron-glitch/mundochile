          ...
          {tab==="general"&&<>
            ...
            {(form.modalidad==="presencial"||form.modalidad==="hibrido")&&<>
              <div style={{marginBottom:"10px"}}>
                ...
                <div style={{display:"flex",gap:"6px"}}>
                  {(()=>{const TL={hotel:"Hotel",centro_eventos:"Centro de eventos",universidad:"Universidad",edificio_corporativo:"Edificio corporativo",oficina_cliente:"Oficina del cliente",planta_produccion:"Planta de producción",faena_minera:"Faena minera",ministerio:"Ministerio",edificio_gobierno:"Edificio de gobierno",otro:"Otro"};const fn=l=>l.nombre;const fnDisp=l=>l.tipo&&TL[l.tipo]?`${TL[l.tipo]} – ${l.nombre}`:l.nombre;return(<select style={S.sel} value={form.lugar||""} onChange={e=>setF("lugar",e.target.value)}><option value="">Seleccionar lugar…</option>{lugares.filter(l=>l.activo!==false).map(l=><option key={l.id} value={fn(l)}>{fnDisp(l)}</option>)}{form.lugar&&!lugares.filter(l=>l.activo!==false).find(l=>fn(l)===form.lugar)&&<option value={form.lugar}>{form.lugar}</option>}</select>);})()}
                  <button onClick={()=>onNuevoLugar&&onNuevoLugar(l=>{const TL={hotel:"Hotel",centro_eventos:"Centro de eventos",universidad:"Universidad",edificio_corporativo:"Edificio corporativo",oficina_cliente:"Oficina del cliente",planta_produccion:"Planta de producción",faena_minera:"Faena minera",ministerio:"Ministerio",edificio_gobierno:"Edificio de gobierno",otro:"Otro"};const fn=l.nombre;setF("lugar",fn);if(onLugarCreado)onLugarCreado();})} style={{padding:"0",width:"42px",height:"42px",background:"#3B82F6",color:"#FFFFFF",border:"none",borderRadius:"8px",cursor:"pointer",fontSize:"20px",fontWeight:"300",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,lineHeight:1}}>+</button>
                </div>
              </div>
