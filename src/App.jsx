        ...
        <div data-modal-scroll style={{overflowY:"auto",flex:1,padding:"20px 24px"}}>
          ...
          {tab==="interpretes"&&!esMultidia&&<>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"20px"}}>
              ...
              <button onClick={()=>{const ni=form.asignaciones.length;addAsig();requestAnimationFrame(()=>requestAnimationFrame(()=>{const el=document.getElementById(`asig-s-${ni}`);const sc=document.querySelector('[data-modal-scroll]');if(el&&sc)sc.scrollTop+=el.getBoundingClientRect().top-sc.getBoundingClientRect().top-16;}));}} style={S.btnA}>+ Agregar intérprete</button>
            </div>
            ...
            {form.asignaciones.length>0&&<button onClick={()=>{const ni=form.asignaciones.length;addAsig();requestAnimationFrame(()=>requestAnimationFrame(()=>{const el=document.getElementById(`asig-s-${ni}`);const sc=document.querySelector('[data-modal-scroll]');if(el&&sc)sc.scrollTop+=el.getBoundingClientRect().top-sc.getBoundingClientRect().top-16;}));}} style={{...S.btnP,width:"100%",padding:"9px"}}>+ Agregar otro intérprete</button>}
          </>}
              ...
              <div style={{padding:"16px"}}>
                ...
                <div style={{marginTop:"12px",border:"2px solid #E03131",borderRadius:"16px",padding:"16px",background:"rgba(224,49,49,0.06)",marginBottom:"16px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"10px"}}>
                    ...
                    <button onClick={()=>{const ni=(dia.asignaciones||[]).length;addAsig(dIdx);requestAnimationFrame(()=>requestAnimationFrame(()=>{const el=document.getElementById(`asig-${dIdx}-${ni}`);const sc=document.querySelector('[data-modal-scroll]');if(el&&sc)sc.scrollTop+=el.getBoundingClientRect().top-sc.getBoundingClientRect().top-16;}));}} style={{...S.btnP,fontSize:"13px",color:"#2C5CA0",border:"1px solid #869CB8"}}>+ Agregar intérprete</button>
                  </div>
